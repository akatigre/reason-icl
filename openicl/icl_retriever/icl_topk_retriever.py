...
class TopkRetriever(BaseRetriever):
    ...
    def forward(self, dataloader, process_bar=False, information: str = ""):
        res_list   = []
        _loader    = copy.deepcopy(dataloader)
        ...
        for entry in _loader:
            with torch.no_grad():
                ...
                raw_text = self.tokenizer.batch_decode(
                    entry["input_ids"], skip_special_tokens=True
                )
                embeds = self.model.encode(raw_text, show_progress_bar=False)
                if isinstance(embeds, torch.Tensor):
                    embeds = embeds.cpu().numpy()

            res_list.extend(
                {"embed": r, "metadata": m} for r, m in zip(embeds, metadata.data)
            )
        return res_list
